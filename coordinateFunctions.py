...
def RandomRotate(Ls,Vs,ir=True,theta=None):#shape=[1,N,3]
    if ir:
        theta = np.random.rand(Ls.shape[1])
    elif theta==None:
        theta = np.random.rand()
    theta *= np.pi*2
    sinTheta = np.sin(theta)
    cosTheta = np.cos(theta)
    Ls0 = Ls[:,:,0]*cosTheta - Ls[:,:,1]*sinTheta
    Ls1 = Ls[:,:,0]*sinTheta + Ls[:,:,1]*cosTheta
    Ls[:,:,0] = Ls0
    Ls[:,:,1] = Ls1
    Vs0 = Vs[:,:,0]*cosTheta - Vs[:,:,1]*sinTheta
    Vs1 = Vs[:,:,0]*sinTheta + Vs[:,:,1]*cosTheta
    Vs[:,:,0] = Vs0
    Vs[:,:,1] = Vs1
    if ir:
        swapind = np.random.rand(Ls.shape[1])>0.5
        tmp = Vs[:,swapind]
        Vs[:,swapind] = Ls[:,swapind]
        Ls[:,swapind] = tmp
    return Ls,Vs

def LS2SphericalCoordiante(Ls,Vs,ir=True,theta=None):
    if ir:
        theta = np.random.rand(Ls.shape[1])
    elif theta==None:
        theta = np.random.rand()
    Vs[:,:,2] = np.arccos(Vs[:,:,2])/np.pi
    Vs[:,:,1] = np.arctan2(Vs[:,:,1],Vs[:,:,0])/(2*np.pi) + 0.5
    Vs[:,:,1] = (Vs[:,:,1] + theta)%1.0
    Vs = Vs[:,:,1:3]
    Ls[:,:,2] = np.arccos(Ls[:,:,2])/np.pi
    Ls[:,:,1] = np.arctan2(Ls[:,:,1],Ls[:,:,0])/(2*np.pi) + 0.5
    Ls[:,:,1] = (Ls[:,:,1] + theta)%1.0
    Ls = Ls[:,:,1:3]
...
def RusinkToDirections(phi_d,theta_h,theta_d):
    ...
    if not np.isnan(angl):
        # if theta_o is 0, angl is nan because of normalize function
...
#Out:   Rusinkiewicz coordinates (phi_d, theta_h, theta_d) phi_h [rad]
...
def DirectionsToRusinkLocal(a,b,norm):
	a = np.reshape(normalize(a),(-1,3))
	b = np.reshape(normalize(b),(-1,3))
	norm = np.reshape(normalize(norm),(-1,3))
	H = normalize((a+b)/2)
	theta_h = np.arccos(np.clip(np.sum(H*norm,-1), -1, 1))
	theta_d = np.arccos(np.clip(np.sum(H*a, -1), -1, 1))
	phi_d = np.arccos(np.clip(np.sum(normalize(H * np.sum(norm*H, -1).reshape(-1,1) - norm)*normalize(b-a),-1), -1, 1))
...
#In:    Direction vectors to view/lightsource
#Out:   Rusinkiewicz coordinates (phi_d, theta_h, theta_d), phi_h  [rad]
def DirectionsToRusink_BTF(a,b):
    a = np.reshape(normalize(a),(-1,3))
    b = np.reshape(normalize(b),(-1,3))
    H = normalize((a+b)/2)   
    theta_h = np.arccos(H[:,2])   
    ...
    theta_d = np.arccos(diff[:,2])
    phi_d = np.mod(np.arctan2(diff[:,1],diff[:,0]),np.pi)
    return np.column_stack((phi_h,phi_d,theta_h,theta_d))
